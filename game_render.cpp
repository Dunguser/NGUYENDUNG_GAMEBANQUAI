...
void render_game_3 ()
{
    ...
    gBackground3.Render(gRenderer, nullptr);                 // in anh background
    tamdung.Render( gRenderer, nullptr);
    map_data = game_map_3.GetMap();                           //xu li di chuyen, va cham map
    ...
    if ( player_nvc.get_x_hientai() >= 6000 && win_game == false)      // qui vuong
    {
        quivuong.xulibossbandan(gRenderer);
        quivuong.SetMapXY(map_data.start_x_, map_data.start_y_);
        quivuong.DICHUYENTHEO_NVC(player_nvc, gRenderer);
    ...
    game_map_3.SetMap( map_data );                                      // cap nhat vi tri moi cho start_x_, start_y_
    game_map_3.DrawMap( gRenderer  );                                    //ve ban do
